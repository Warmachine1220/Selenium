...
//1st screenflow
		...
		CRRScandofaic.Select("SALUTATION", Salutation);
		CRRScandofaic.Select("SUFFIX", suffix);
...
//Employment section
		...
		if (Emplytype != "Unemployed") {
			...
			WebElement Nature_work = driver.findElement(By.name("cust_2835"));
...
//ID
		...
		ID.sendKeys("Philsys ID");
		Thread.sleep(1000);
		ID.sendKeys(Keys.DOWN);
		ID.sendKeys(Keys.ENTER);
		Thread.sleep(500);

		driver.findElement(By.name("cust_486")).sendKeys("qwert" + f); // id no
